...
def get_id_estudante(nome):
    for i in get_estudante():
        if i['nome'] == nome:
            return i['id']
...
def main(page: ft.Page):    
    ...
    def alterar(e):
        ...
        response = requests.put("http://127.0.0.1:8000/cursos/estudante/", json=data)
    ...
    '''Menus Dropdow'''
    ft.Container()
    txt_estudante=ft.Text('Estudante:',color=ft.colors.AMBER)
    estudante=ft.Dropdown(
        options=[
            ft.dropdown.Option(i['nome']) for i in get_estudante()
        ]
    )
    ...
    btn_estudante=ft.ElevatedButton('ALTERAR', on_click=alterar)
